...
SpaceSchema.index({ averageRating: -1, reviewCount: -1 });
SpaceSchema.index({ totalBookings: -1 });
SpaceSchema.index({ totalRevenue: -1 });
SpaceSchema.index({ totalViews: -1 });
SpaceSchema.index({ capacity: 1 });
SpaceSchema.index({ 'location.city': 1 });
SpaceSchema.index({ 'location.state': 1 });
SpaceSchema.index({ status: 1, isActive: 1 });
...
SpaceSchema.index({ category: 1, 'location.city': 1, isActive: 1 });
SpaceSchema.index({ 'pricing.basePrice': 1, capacity: 1, isActive: 1 });
SpaceSchema.index({ averageRating: -1, totalBookings: -1, isActive: 1 });
SpaceSchema.index({ ownerId: 1, status: 1, createdAt: -1 });
SpaceSchema.index({ 'location.city': 1, category: 1, averageRating: -1 });
...
SpaceSchema.virtual('isPopular').get(function() {
  return this.totalBookings >= 50 && this.averageRating >= 4.0;
});
...
SpaceSchema.virtual('pricePerHourFormatted').get(function() {
  return `₹${(this.pricing.basePrice / 100).toLocaleString('en-IN')}`;
});

SpaceSchema.virtual('pricePerDayFormatted').get(function() {
  return `₹${(this.pricing.dailyRate / 100).toLocaleString('en-IN')}`;
});
...
SpaceSchema.virtual('staffActivities', {
  ref: 'StaffActivity',
  localField: '_id',
  foreignField: 'spaceId',
  justOne: false
